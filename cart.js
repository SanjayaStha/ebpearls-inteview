...
function ready() {
    ...
    for(vari=0;i<removeCartItemsButtons.length; i++){
...
function purchaseClicked(){
    ...
    let address = document.getElementsByClassName("user-address")[0].value
    let contactno = document.getElementsByClassName("user-contact")[0].value
    var cartItems = document.getElementsByClassName('cart-items')[0]
    // alert(cartItems.innerText)
    ...
    if(name=="" || address=="" || contactno=="" ){
        alert(`Please fill all details\n ${name}, ${address} ${contactno} ${cartItems}`)
        return
    }
